...
if __name__ == '__main__':
    ...
    # copied from region2.py, hence variable names are reused

    doc = Doc.from_disk('sample2.json')
    ...
    print(region.str_spans())
    assert region.str_spans() == spans2, f'Act: {region.str_spans()} Exp:{spans2}'

    print("\n=>3. Printing removing idx spans")
    rm_idx_config = TextConfig(rm_labels=["idx"])
    print(region.line_text(rm_idx_config))
    assert region.line_text(rm_idx_config) == line3, f'Act: {region.line_text(rm_idx_config)} Exp: {line3}'
    ...
    print(region.str_spans())    
    assert region.str_spans() == spans8, f'\nAct: {region.str_spans()}\n\nExp: {spans8}'
    ...
    assert words_texts_str == word_texts1, f'\nAct: {words_texts_str}\nExp: {word_texts1}'
    ...
    words_texts_str = ", ".join(word_texts)
    print(words_texts_str)
    assert words_texts_str == word_texts2, f'Act: {words_texts_str} Exp: {word_texts2}'
    ...
    word_texts = [w.text for w in region.get_words_in_spans([span])]
    words_texts_str = ", ".join(word_texts)
    print(words_texts_str)
    ...
    rm_boundary_config = TextConfig(rm_labels=['boundary'])
    print(region.line_text(rm_boundary_config))
    assert region.line_text(rm_boundary_config) == line_B2, f'Act: {region.line_text(rm_boundary_config)} {line_B2}'
    ...
    print(region.str_spans())    
    print(words_texts_str)
    assert words_texts_str == word_texts_B2, f'\nAct: {words_texts_str}\nExp: {word_texts_B2}'
