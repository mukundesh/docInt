...
def get_layoutlm_json(doc, page_idx):
    ...
    def build_word(word):
        return {'text': w.text, 'box': flatten_box(word.box)}
    
    start_idx, entities = 0, []
    for label, region in doc[page_idx].layoutlm:
        entity = { 'id': start_idx, 'text': region.line_text,
                   'box': flatten_box(region.shape), 'label': label,
                   'linking': [], 'words': []
                  }

        entity['words'] = [ build_word(w) for w retion.iter_word_lines_words() ]
