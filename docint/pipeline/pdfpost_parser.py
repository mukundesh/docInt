...
from ..hierarchy import Hierarchy, MatchOptions, HierarchySpanGroup
...
class PostParser:
    ...
    def handle_juri(self, post_str, dept_sg, role_sg, juri_sgs):
        ...
        if is_comm_role:
            ...
            assert len(sel_sgs) <= 1, f"label: {label} {len(sel_sgs)}  span_groups found, {post_str} {Hierarchy.to_str(sel_sgs)}"
            return sel_sgs
        else:
            sub_path = ['jurisdiction', 'policing']
            sel_sgs = HierarchySpanGroup.select_path_match(juri_sgs, sub_path)
    ...
    def parse(self, post_words, post_str, post_path, rank=None):
        ...
        for (field, hierarchy) in self.hierarchy_dict.items():
            ...
            if field == "role" and not span_groups and post_str.lower().startswith('circle'):
    ...
    def get_rank(self, doc):
        ...
        reg_dict = {'Additional Superintendent of Police': 'addl[\. ]*s[\.]?p[\.]?',
