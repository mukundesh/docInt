...
from .page_image_builder_raster import build_page_image as build_page_image_raster
...
def build_page_image(page, pdf_page, image_dir):
    ...
    image_root = image_dir / Path(page.doc.pdf_stem) / "embedded"
    image_path = image_dir / Path(page.doc.pdf_stem) / f"embedded-{page_num:03d}-000.png"
    ...
    def __call__(self, doc, pipe_config={}):
        ...
        for (page, pdf_page) in zip(doc.pages, pdf.pages):
            if len(pdf_page.images) == 1:
                ...
                page_image = build_page_image_raster(page, pdf_page, self.image_dir)
            page.page_image = page_image
