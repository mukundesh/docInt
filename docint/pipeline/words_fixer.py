...
class WordsFixer:
    def __init__(self, item_name, conf_dir, conf_stub, pre_edit, dict_file, lv_dist_cutoff, ignore_paren_len, unicode_file, officer_at_start):
        ...
        u_lines = [l.split() for l in Path(self.unicode_file).read_text().split('\n') if l.strip()]
    ...
    def mark_names(self, list_item):
        ...
        if boundary_char and boundary_char != ' ':
            name = line_text[first_span.start:first_span.end]
            try:
                new_end = line_text.index(' ', first_span.end)
                first_span.end = new_end
                new_name = line_text[first_span.start:first_span.end]
    ...
    def fix_list(self, list_item):
        ...
        unicode_count = self.fix_unicode(list_item)
        ...
        punct_count = self.blank_punct(list_item)
