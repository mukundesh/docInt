...
class ListFinder:
    ...
    def get_rotated_word_lines(self, page, rotation_config):
        ...
        if abs(angle) > rotation_config["rotation_min_angle"]:
            self.lgr.info(
                f"Rotated page {page.page_idx} strategy: {strategy} angle: {angle}"
            )
        ...
        for page in doc.pages:
            ...
            if hasattr(page, "arranged_word_lines_idxs"):
                ...
                word_lines = words_in_lines(
                    page, newline_height_multiple=nl_ht_multiple
                )
            ...
            if len(num_markers) >= self.min_markers_onpage:
                page.list_items = self.find_inpage(
                    word_lines, num_markers, page.page_idx
                )
            else:
                page.list_items = []
