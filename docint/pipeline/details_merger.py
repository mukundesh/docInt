...
class OfficerHistory:
    ...
    def alias_name_match(selif, d_name):
        for name in self.names:
            if alig_match(name, d_name):
                return True
        return False
    ...
    def fuzzy_name_exact_date_match(self, d_name, d_birth_date, name_cutoff=1):
        for name, birth_date in product(self.names, self.birth_dates):
            if levenshtein(d_name, name, name_cutoff) <= name_cutoff and d_birth_date == birth_date:
                return True
        return False
        ...
        if not self.officer_id and detail.officer.officer_id:
            ...
            for d in self.details:
                if not d.officer.officer_id:
                    ...
                    assert self.officer_id == d.officer.officer_id, f'mismatch in officer_id: {self.officer_id} != {d.officer.officer_id}'
        elif self.officer_id and detail.officer.officer_id:
            if self.officer_id != detail.officer.officer_id:
                assert False, f"HUGE ERROR: {self.officer_id} != {detail.officer.officer_id} {self.name} <-> {detail.officer.name}"
    ...
    def build_OfficerID(self):
        ...
        oid = OfficerID(officer_id=o.officer_id,
                         salut=o.salut,
                         name=o.name,
...
class DetailsMerger:
    ...
    def build_mismatch(self, mis_dict):
        tuples = []
        for l in mis_dict['mis_matches']:
            tuples.append(tuple(sorted([fix_name(l[0]), fix_name(l[1])])))
        return set(tuples)
    ...
    def find_matching_detail(self, officer_history, order, o_idx):
                ...
                for d_name, d in valid_details:
                    ...
                    if d_post and oh_last_post.post_id == d_post.post_id:
                        if oh.exact_name_match(d_name):
                            ...
                            names = "|".join(oh.names)
                            self.lgr.info(f'\t\tMissed-post: {names}<->{d_name} {oh_last_post.post_str}<->{d_post.post_str}')
            return None
        ...
        def fuzzy_name_exact_post_match():
            if oh_last_post:
                for d_name, d in valid_details:
                    d_post = get_before_post(d)
                    if d_post and oh_last_post.post_id == d_post.post_id and oh.fuzzy_name_match(d_name):
                        return d
            return None
        ...
        def fuzzy_name_match():
            if not self.is_duplicate(officer_history):
                for d_name, d in valid_details:
                    if oh.fuzzy_name_match(d_name) and allowed_name_match(oh, d_name) and (fix_name(d_name) not in self.duplicate_nows_names):
                        return d
            return None
        ...
        if detail:
            if self.detail_added(order, detail):
                ...
                self.lgr.info(f'\tFound: {order_id}:{o_idx} officer_id_match {oh.names_str}<->{detail.officer.name} {oh.birth_date_str}<->{detail.officer.birth_date}')
                return detail

        detail = exact_name_exact_date_match()
        if detail:
            if self.detail_added(order, detail):
                self.lgr.info(f'HUGE DUPLICATE DETAIL ERROR exact_name_exact_date_match {order.order_id}.{detail.detail_idx}')
            else:
        ...
        if detail:
            self.lgr.info(f'\tFound: {order_id}:{o_idx} exact_name_fuzzy_date_match {oh.birth_date_str}<->{detail.officer.birth_date}')
            return detail

        detail = fuzzy_name_exact_date_match()
        if detail:
            self.lgr.info(f'\tFound: {order_id}:{o_idx} fuzzy_name_exact_date_match {oh.names_str}<->{detail.officer.name}')
            return detail

        detail = fuzzy_name_fuzzy_date_match()
        if detail:
            self.lgr.info(f'\tFound: {order_id}:{o_idx} fuzzy_name_fuzzy_date_match {oh.names_str}<->{detail.officer.name} {oh.birth_date_str}<->{detail.officer.birth_date}')
            return detail

        oh_last_post = oh.get_last_post()
        detail = exact_name_exact_post_match()
        if detail:
            self.lgr.info(f'\tFound: {order_id}:{o_idx} exact_name_exact_post_match {oh.names_str}<->{detail.officer.name}')
            return detail

        detail = fuzzy_name_exact_post_match()
        if detail:
            self.lgr.info(f'\tFound: {order_id}:{o_idx} fuzzy_name_exact_post_match {oh.names_str}<->{detail.officer.name}')
            return detail

        detail = align_name_exact_date_match()
        if detail:
            self.lgr.info(f'\tFound: {order_id}:{o_idx} align_name_exact_date_match {oh.names_str}<->{detail.officer.name} {oh.birth_date_str}<->{detail.officer.birth_date}')
            return detail
    ...
    def get_officer_histories(self, orders):
        ...
        for (o_idx, order, d_idx, detail) in iter_valid_details(orders):
            ...
            o_history = OfficerHistory(orders=[order], details=[detail], order_idxs=[o_idx], officer_id=officer_id)
    ...
    def pipe(self, docs, **kwargs):
        ...
        self.order_name_birthdate_dict = dict(( ((o.order_id, fix_name(d.officer.name), d.officer.birth_date), d) for o in civil_orders for d in o.details if d.officer and d.officer.birth_date))
        ...
        noid_post_error_details = len([d for o in non_civil_orders for d in o.details if (not d.officer.officer_id) and has_error_posts(d)])
        ...
        print(f"#details: {total_details} #withid_details: {withid_details} #OHs: {len(officer_histories)} #noid_OHs: {len(noid_OHs)} #noid_dup_OHs: {len(dup_noid_OHs)}")
